...
// --- ASYNC BUTTON HELPERS ---
const asyncButtonStates = new WeakMap();

/**
 * Mark a button as busy while an async action runs.
 * Returns a token that can be used to safely reset the button later.
 ...
 * @returns {symbol|null}
 */
function setButtonBusy(button, spinner, textElement, busyText) {
  if (!button) return null;

  let state = asyncButtonStates.get(button);
  if (!state) {
    state = { token: null, timeout: null };
  ...
  const token = Symbol('button-op');
  state.token = token;

  button.disabled = true;
  if (spinner) showSpinner(true, spinner);
  if (textElement && busyText !== undefined) {
    setButtonText(textElement, busyText);
  }
...
/**
 * Reset a button after an async action if the token still matches.
 * Spinner is hidden immediately; the button can be optionally re-enabled after a delay.
 * @param {HTMLButtonElement} button
 * @param {HTMLElement|null} spinner
 * @param {HTMLElement|null} textElement
 * @param {string} defaultText
 * @param {symbol|null} token
 * @param {{delay?: number, keepDisabled?: boolean}=} options
 */
function setButtonIdle(button, spinner, textElement, defaultText, token, options = {}) {
  if (!button || !token) return;

  const state = asyncButtonStates.get(button);
  ...
  const { delay = 0, keepDisabled = false } = options;

  if (spinner) showSpinner(false, spinner);

  const applyReset = () => {
    if (textElement && defaultText !== undefined) {
      setButtonText(textElement, defaultText);
    }
    if (!keepDisabled) {
...
/**
 * Immediately restore a button to its default state, cancelling any pending timers.
 * @param {HTMLButtonElement} button
 * @param {HTMLElement|null} spinner
 * @param {HTMLElement|null} textElement
 * @param {string} defaultText
 */
function forceResetButton(button, spinner, textElement, defaultText) {
  if (!button) return;
